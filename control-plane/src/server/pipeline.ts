...
export async function runPipeline({
  ...
  return withLogContext(
    ...
    async () => {
      log.info("pipeline.started", { inputKind: input.kind })
      ...
      const begin = (id: StageId) => {
        ...
        log.info("pipeline.stage.started", { stage: id })
      }
      const finish = (id: StageId, detail?: string) => {
        ...
        log.info("pipeline.stage.completed", {
          stage: id,
          durationMs: Date.now() - (stageStarted.get(id) ?? Date.now()),
          detail,
        })
      }
      ...
      try {
        ...
        if (isVoice && audioPath) {
          begin("stt")
          // Always auto-detect, even when the builder picked a language. Pinning
          // `language_code` makes saaras decode *into* that language, so a Kannada
          // note on a Tamil-configured agent came back as Tamil-looking text and
          // the agent dutifully replied in Tamil. The configured language is a
          // fallback for when detection returns nothing, not a filter.
        ...
        const result = await runTurn(
          ...
          userText,
          (step) => {
            const existing = agentStage.steps!.findIndex(
              (s) => s.id === step.id
            )
        ...
        log.info("pipeline.completed", {
          ...
          replyKind: replyAudio ? "voice" : "text",
        })
      } catch (error) {
